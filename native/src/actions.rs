...
use spectape_core::audio::FlowOptions;
use spectape_core::consistency::{check_consistency, Severity};
use spectape_core::programs::{detect_programs, Program};
use spectape_core::types::{Block, Body};
use spectape_core::writer::{required_version, serialize_tzx};
...
pub fn play_tape(app: &mut App, side: Side, from_cursor: bool) {
    ...
    let mut order = spectape_core::audio::playback_order(&t.blocks, FlowOptions::default());
