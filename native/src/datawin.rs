...
use spectape_core::bits::{
    add_bits, drop_bits, flip_bytes, join_bits, shift_left_bits, shift_right_bits, total_bits, BitData,
};
use spectape_core::content::detect_content;
use spectape_core::spectrum::basic::{basic_to_text, list_basic, list_variables, BasicOptions};
use spectape_core::spectrum::charset::{dump_char, zx_char};
use spectape_core::spectrum::screen::{has_flash, render_screen, ScreenOptions, SCREEN_SIZE};
use spectape_core::spectrum::z80dis::{disassemble, DisOptions};
use spectape_core::types::{Block, Body, Uid};
...
impl DataWin {
    pub fn new(blocks: &[Block], side: Side, uids: Vec<Uid>) -> DataWin {
        ...
        let view = match guess.kind {
            spectape_core::content::ContentKind::Screen if single => ViewAs::Screen,
            spectape_core::content::ContentKind::Basic if single => ViewAs::Basic,
            _ => ViewAs::Dump,
        };
        DataWin {
            side,
