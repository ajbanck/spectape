...
//!
//! `src/state/store.ts` persists `spectape.theme`, `spectape.hexBytes`,
//! `spectape.zeroBased` and `spectape.emulator`, and `TapePane.tsx` the two
...
pub fn config_dir() -> Option<PathBuf> {
    ...
    let dir = std::env::var_os("APPDATA").map(|a| PathBuf::from(a).join("SpecTape"));
...
impl Settings {
    pub fn load() -> Settings {
        ...
        if let Some(v) = get("spectape.theme") {
            s.theme = Theme::from_name(v);
        }
        s.hex_bytes = get("spectape.hexBytes") == Some("1");
        s.zero_based = get("spectape.zeroBased") == Some("1");
        if let Some(v) = get("spectape.emulator.program") {
            s.emulator_program = v.to_string();
        }
        if let Some(v) = get("spectape.emulator.args") {
            s.emulator_args = v.to_string();
        }
        if let Some(v) = get("spectape.editorHeight").and_then(|v| v.parse::<f32>().ok()) {
            if v >= 140.0 {
                s.editor_height = v;
            }
        }
        if let Some(v) = get("spectape.paneSplit").and_then(|v| v.parse::<f32>().ok()) {
            if v > 0.0 && v < 1.0 {
                s.pane_split = v;
            }
        }
    ...
    pub fn save(&self) {
        ...
        let text = format!(
            "spectape.theme={}\n\
             spectape.hexBytes={}\n\
             spectape.zeroBased={}\n\
             spectape.emulator.program={}\n\
             spectape.emulator.args={}\n\
             spectape.editorHeight={:.0}\n\
             spectape.paneSplit={:.3}\n",
            self.theme.name(),
