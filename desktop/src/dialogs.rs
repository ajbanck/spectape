...
use spectape_core::audio::{playback_order, FlowOptions, RenderOptions, TSTATES_PER_SEC};
use spectape_core::consistency::{check_consistency, Severity};
use spectape_core::describe::describe_block;
use spectape_core::programs::detect_programs;
use spectape_core::types::{block_name, create_body, Block, CREATABLE_IDS};
use spectape_core::writer::{save_version, serialize_tzx};
...
pub fn draw(app: &mut App, ctx: &egui::Context) {
    ...
    let response = egui::Modal::new(egui::Id::new("spectape-dialog"))
        .frame(
...
fn about_body(ui: &mut Ui, tok: &Tokens) -> Outcome {
    ui.label(RichText::new("SpecTape").strong());
...
fn export_wav(app: &mut App, s: &WavState, order: &[u32]) {
    ...
    let wav = spectape_core::audio::encode_wav(&samples, s.rate, s.bits);
