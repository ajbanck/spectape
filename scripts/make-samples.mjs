...
//
// Produces:
//   SpecTape demo.tzx            every block type the editor can create, in one tape
//   SpecTape demo (variant).tzx  a variation of the same program for the compare modes
//   SpecTape demo.tap            the TAP export of the demo tape
...
try {
  ...
  const demoBasic = program([
    line(10, T.REM, ...latin1(' SpecTape demo tape ')),
  ...
  const variantBasic = program([
    line(10, T.REM, ...latin1(' SpecTape demo tape, variant ')),
  ...
  // ---- a screen: the word SPECTAPE in a chunky 5x7 font, with colour bands ------------
  ...
  const demoScreen = screen('SPECTAPE', 7, 1, [1, 1, 5, 5, 4, 4, 6, 6, 2, 2, 3, 3]);
  ...
  const demo = [
    block(0x32, {
      entries: [
        { type: 0x00, text: 'SpecTape demo' },
        { type: 0x01, text: 'SpecTape project' },
    ...
    block(0x30, { text: 'End of the SpecTape demo tape.' }),
    block(0x20, { pause: 0 }),
  ];

  // ---- the variant: same program, different loader, for the compare modes -------------

  const variantScreen = screen('SPECTAPE', 6, 0, [0, 0, 2, 2, 0, 0, 3, 3]);
  const direct = () => {
  ...
  const variant = [
    block(0x32, {
      entries: [
        { type: 0x00, text: 'SpecTape demo (variant)' },
        { type: 0x01, text: 'SpecTape project' },
        { type: 0x03, text: '2026' },
  ...
  write('SpecTape demo.tzx', verify('demo', demo));
  write('SpecTape demo (variant).tzx', verify('variant', variant));
  write('SpecTape demo.tap', serializeTap(demo).bytes);
  if (failed) {
