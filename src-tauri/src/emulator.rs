...
fn resolve(program: &str) -> Result<Target, String> {
    ...
    #[cfg(target_os = "macos")]
    if p.extension().map_or(false, |e| e.eq_ignore_ascii_case("app")) {
...
fn temp_folder() -> Result<PathBuf, String> {
    let dir = std::env::temp_dir().join("spectape-emulator");
    std::fs::create_dir_all(&dir).map_err(|e| format!("Cannot create {}: {e}", dir.display()))?;
    if let Ok(entries) = std::fs::read_dir(&dir) {
        let old = SystemTime::now() - Duration::from_secs(24 * 3600);
        for e in entries.flatten() {
            if e.metadata().and_then(|m| m.modified()).map_or(false, |t| t < old) {
