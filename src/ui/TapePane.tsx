...
function loadEditorHeight(): number {
  try {
    const v = Number(localStorage.getItem('spectape.editorHeight'));
...
const SPLIT_KEY = 'spectape.paneSplit';
...
export function TapePane({ side, grow = 1 }: { side: Side; grow?: number }) {
  ...
  const onSplitterDown = (e: PointerEvent) => {
    ...
    const up = () => {
      ...
      el.removeEventListener('pointercancel', up);
      setDragging(false);
      try { localStorage.setItem('spectape.editorHeight', String(Math.round(last))); } catch { /* ignore */ }
    };
    el.addEventListener('pointermove', move);
    el.addEventListener('pointerup', up);
    el.addEventListener('pointercancel', up);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onDblClick={() => { setEditorH(EDITOR_DEFAULT); try { localStorage.setItem('spectape.editorHeight', String(EDITOR_DEFAULT)); } catch { /* ignore */ } }}
      />
