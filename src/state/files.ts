...
// adapter, and routing files that arrive from dialogs, drops or the OS.
...
export function loadBytes(side: Side, name: string, bytes: Uint8Array, insertAtCursor = false, path?: string) {
  let parsed;
  try {
  ...
  if (insertAtCursor) {
    ...
    tapes[side].value = {
      ...emptyTape(name), path: path ?? null, blocks: parsed.blocks, saved: parsed.blocks, cursor: parsed.blocks.length ? 0 : -1,
...
/** Save bytes through the platform (download on the web, native dialog on the desktop). */
...
/** Save as TZX. With saveAs=false the desktop build writes back to the file it was opened from. */
export async function saveTzx(side: Side, saveAs = false) {
  const t = tapes[side].value;
  const v = saveVersion(t.blocks, t.loadedVersion);
  const bytes = serializeTzx(t.blocks, v);
  const inPlace = !saveAs && t.path && /\.tzx$/i.test(t.path) ? t.path : undefined;
  const p = await platform();
  const r = await p.saveFile({ suggestedName: stem(t.name) + '.tzx', filters: [{ name: 'TZX tape image', extensions: ['tzx'] }], bytes, path: inPlace });
  if (!r) return;
  markSaved(side, { loadedVersion: v, name: r.name, path: r.path ?? tapes[side].value.path });
...
export async function saveTap(side: Side) {
  ...
  if (skipped.length) {
    ...
    markSaved(side, { name: r.name, path: r.path ?? null });
    setStatus(`Saved ${r.name}`);
  }
}

/** Load already-read files (from a dialog, a drop or the OS) into a tape. */
export function openTapeFiles(side: Side, files: OpenedFile[], insertAtCursor = false) {
  for (const f of files) loadBytes(side, f.name, f.bytes, insertAtCursor, f.path);
}
...
/** Route files the OS asked us to open: left tape unless it has unsaved work, then right. */
export function openWithFiles(files: OpenedFile[]) {
  let side: Side = tapes[0].value.dirty ? 1 : 0;
  for (const f of files) {
    loadBytes(side, f.name, f.bytes, false, f.path);
    side = side === 0 ? 1 : 0;
  }
}
